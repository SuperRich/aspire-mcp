...
// Define AWS Aspire data based on video insights
const awsAspireData = {
  overview: `# AWS and .NET Aspire Integration

.NET Aspire provides powerful integration capabilities with AWS services, enabling developers to build cloud-native applications that leverage the AWS ecosystem while maintaining the developer-friendly experience of .NET Aspire.`,

  keyTakeaways: `## Key Takeaways from AWS and .NET Aspire Integration

1. **Container Deployment** - .NET Aspire applications can be deployed to AWS container services like ECS and EKS, with built-in support for containerization
2. **AWS SDK Integration** - Aspire integrates with AWS SDK, providing automatic configuration, health checks, and telemetry for AWS services
3. **Infrastructure as Code** - Aspire components can be deployed using AWS CloudFormation or CDK, enabling infrastructure as code practices
4. **Local Development** - You can develop locally with .NET Aspire while using AWS service emulators or actual AWS services
5. **Observability** - Aspire's built-in observability works with AWS monitoring services like CloudWatch`,

  commonServices: `## Common AWS Services Used with .NET Aspire

| AWS Service | Description | Integration Type |
|-------------|-------------|-----------------|
| Amazon S3 | Object storage service | Storage integration |
| Amazon RDS | Managed relational database service | Database integration |
| Amazon DynamoDB | NoSQL database service | Database integration |
| Amazon SQS | Message queuing service | Messaging integration |
| Amazon SNS | Notification service | Messaging integration |
| AWS Lambda | Serverless compute service | Compute integration |
| Amazon CloudWatch | Monitoring and observability service | Observability integration |`,

  codeExamples: `## Code Examples

### Adding AWS S3 Integration

\`\`\`csharp
// In the AppHost project
var s3 = builder.AddAmazonS3("s3");

var myService = builder.AddProject<Projects.MyService>("myService")
    .WithReference(s3);

// In the service project
builder.AddAmazonS3Client("s3");
...
\`\`\`csharp
// In the AppHost project
var dynamoDB = builder.AddAmazonDynamoDB("dynamodb");

var myService = builder.AddProject<Projects.MyService>("myService")
    .WithReference(dynamoDB);

// In the service project
builder.AddAmazonDynamoDBClient("dynamodb");

// Then in your code
public class MyService(IAmazonDynamoDB dynamoDBClient)
{
...
1. **AWS App Runner** - Deploy containerized .NET Aspire applications with minimal configuration
2. **Amazon ECS** - Deploy to ECS for container orchestration with more control
3. **Amazon EKS** - Use Kubernetes for complex microservice architectures
4. **AWS Fargate** - Serverless compute for containers without managing servers

The deployment manifest generated by .NET Aspire can be used with AWS CloudFormation or CDK to provision the required AWS resources.`
};
...
// Add a tool for AWS and .NET Aspire integration
server.tool("aws_aspire",
  { 
    section: z.enum(["overview", "key-takeaways", "common-services", "code-examples", "deployment", "all"]).optional()
  },
  async ({ section }) => {
    let content = "";
    
    if (!section || section === "overview" || section === "all") {
      content += awsAspireData.overview + "\n\n";
    }
    
    if (!section || section === "key-takeaways" || section === "all") {
      content += awsAspireData.keyTakeaways + "\n\n";
    }
    
    if (!section || section === "common-services" || section === "all") {
      content += awsAspireData.commonServices + "\n\n";
    }
    
    if (!section || section === "code-examples" || section === "all") {
      content += awsAspireData.codeExamples + "\n\n";
    }
    
    if (!section || section === "deployment" || section === "all") {
      content += awsAspireData.deploymentOptions + "\n\n";
    }
    
    content += "Source: [AWS and .NET Aspire Integration](https://www.youtube.com/watch?v=8WxvA_xCt2c&t=1901s)";
